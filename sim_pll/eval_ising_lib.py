...
class EvaluateAndPlotIsingMachineSimulation:
	...
	"""
	def __init__(self, pool_data: dict, phase_wrap=0, number_of_histogram_bins=25, prob_density=False, order_param_solution=0.0, number_of_expected_oscis_in_one_group=10) -> None:
		"""
		Args:
			pool_data: [dict] contains all the data of the simulations to be evaluated and the settings
			phase_wrap: [integer] whether phases are wrapped into the interval 0) [0, 2*pi), 1) [-pi, pi), or 2) [-pi/2, 3*pi/2)
