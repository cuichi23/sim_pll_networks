...
# #*******************************************************************************
[lineOmegStab] = ax.plot(np.asarray(w/(2.0*np.pi))*solveLinStab(globalFreq(w, K, tauf, v, digital, maxp)['Omeg'],globalFreq(w, K, tauf, v, digital, maxp)['tau'], tauf, K, tauc, v, digital,maxp, al)['tauStab']* solveLinStab(globalFreq(w, K, tauf, v, digital, maxp)['Omeg'],globalFreq(w, K, tauf, v, digital, maxp)['tau'], tauf, K, tauc, v, digital,maxp, al)['OmegStab'],
...
[lineOmegUnst] = ax.plot(np.asarray(w/(2.0*np.pi))*solveLinStab(globalFreq(w, K, tauf, v, digital, maxp)['Omeg'],globalFreq(w, K, tauf, v, digital, maxp)['tau'], tauf, K, tauc, v, digital,maxp, al)['tauUnst']*solveLinStab(globalFreq(w, K, tauf, v, digital, maxp)['Omeg'],globalFreq(w, K, tauf, v, digital, maxp)['tau'], tauf, K, tauc, v, digital,maxp,al)['OmegUnst'],
...
# plot grid, labels, define intial values
plt.title(r'The Expansion of 3rd order of the characteristic equation');
plt.grid()
plt.xlabel(r'$\Omega\tau/2\pi$', fontsize=18)
plt.ylabel(r'$2\pi$Re$(\lambda)$, Im$(\lambda)/\omega$', fontsize=18)
...
[lineSigma1] = ax1.plot(np.asarray(w/(2.0*np.pi))*globalFreq(w, K, tauf, v, digital, maxp)['Omeg']*globalFreq(w, K, tauf, v, digital, maxp)['tau'], np.asarray(2.0*np.pi)*solveLinStab(globalFreq(w, K, tauf, v, digital, maxp)['Omeg'],
	globalFreq(w, K, tauf, v, digital, maxp)['tau'], tauf_0, K_0, tauc_0, v_0, digital,maxp,al)['Re'],	linewidth=2, color='red', label=r'Re$(\lambda)$')
[lineGamma1] = ax1.plot(np.asarray(w/(2.0*np.pi))*globalFreq(w, K, tauf, v, digital, maxp)['Omeg']*globalFreq(w, K, tauf, v, digital, maxp)['tau'], np.asarray(2.0*np.pi)*solveLinStab(globalFreq(w, K, tauf, v, digital, maxp)['Omeg'],
...
v_slider      = Slider(v_slider_ax, 'v', 1, 512, valinit=v_0)
# Draw another slider
tauf_slider_ax  = fig.add_axes([0.25, 0.07, 0.65, 0.02], facecolor=axis_color)
tauf_slider     = Slider(tauf_slider_ax, 'tauf', 0.0, 25/w, valinit=tauf_0)#25*(2.0*np.pi/w)
# Draw another slider
K_slider_ax = fig.add_axes([0.25, 0.04, 0.65, 0.02], facecolor=axis_color)
K_slider    = Slider(K_slider_ax, 'K', 0.001*w, 1.5*w, valinit=K_0)
# Draw another slider
tauc_slider_ax  = fig.add_axes([0.25, 0.01, 0.65, 0.02], facecolor=axis_color)
tauc_slider     = Slider(tauc_slider_ax, 'tauc', 0.05*(1.0/w), 10.0*(1.0/w), valinit=tauc_0)

# define an action for modifying the line when any slider's value changes
def sliders_on_changed(val):
	global digital
	lineOmegStab.set_ydata(np.asarray(2.0*np.pi)*solveLinStab(globalFreq(w, K_slider.val, tauf_slider.val, v_slider.val, digital, maxp)['Omeg'],
						globalFreq(w, K_slider.val, tauf_slider.val, v_slider.val, digital, maxp)['tau'], tauf, K_slider.val,
						tauf_slider.val, v_slider.val, digital,maxp, al)['OmegStab']);
	...
	lineOmegUnst.set_ydata(np.asarray(2.0*np.pi)*solveLinStab(globalFreq(w, K_slider.val, tauf_slider.val, v_slider.val, digital, maxp)['Omeg'],
						globalFreq(w, K_slider.val, tauf_slider.val, v_slider.val, digital, maxp)['tau'], tauf, K_slider.val,
						tauf_slider.val, v_slider.val, digital,maxp, al)['OmegUnst']);
