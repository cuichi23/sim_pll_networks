...
dict_pll={
	...
	'coupK': 0.2,																# [random.uniform(0.3, 0.4) for i in range(dict_net['Nx']*dict_net['Ny'])],# coupling strength in Hz float or [random.uniform(minK, maxK) for i in range(dict_net['Nx']*dict_net['Ny'])]
	...
	'transmission_delay': 0.15, 												# value of transmission delay in seconds, float (single), list (tau_k) or list of lists (tau_kl): np.random.uniform(min,max,size=[dict_net['Nx']*dict_net['Ny'],dict_net['Nx']*dict_net['Ny']]), OR [np.random.uniform(min,max) for i in range(dict_net['Nx']*dict_net['Ny'])]
...
tau 	= np.arange(0, 16, 0.05)
wc  	= 2.0*np.pi*np.arange(0.0001, 0.8, 0.6285/(4.0*np.pi))
#tau 	= np.arange(0, 16, 0.001)
#wc  	= 2.0*np.pi*np.arange(0.0001, 1, 0.006285/(4.0*np.pi))
...
for i in range(len(tau)):
	...
	for j in range(len(wc)):
		dict_pll.update({'cutFc': wc[j]/(2*np.pi)})								# set this temporarly to one value -- in Hz
		...
		if wc[j]*fric**2/(2*alpha[i,j]) > fzeta:
			CondStab[i,j] = 0
		elif wc[j]*fric**2/(2*alpha[i,j]) > 1:	#wc[j]/(2*alpha[i,j]) < fzeta and wc[j]/(2*alpha[i,j]) > 1:
			CondStab[i,j] = 1
		else:
			CondStab[i,j] = None
...
paramsDict = {'h': h, 'hp': hp, 'w': w, 'K': K, 'wc': wc, 'fric': fric, 'Omeg': OmegInTauVsFc, 'alpha': alpha, 'CondStab': CondStab,
