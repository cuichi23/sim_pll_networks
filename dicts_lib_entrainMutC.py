...
def getDicts(Fsim: int = 55):
	...
	dict_pll = {
		...
		'noiseVarVCO': [0, 5E-10, 1E-10],										# variance of VCO GWN
		'feedback_delay': 0,													# value of feedback delay in seconds
		'feedback_delay_var': None, 											# variance of feedback delay
		'transmission_delay': 0.349, #0.256, #5.0, 								# value of transmission delay in seconds, float (single), list (tau_k) or list of lists (tau_kl): np.random.uniform(min,max,size=[dict_net['Nx']*dict_net['Ny'],dict_net['Nx']*dict_net['Ny']]), OR [np.random.uniform(min,max) for i in range(dict_net['Nx']*dict_net['Ny'])]
	...
	dict_algo = {
		...
		'paramDiscretization': [16, 16],										# parameter discretization for brute force initial phases and parameter space scans, e.g., for 'listOfInitialPhaseConfigurations' it specifies the number of initial phase differences and the discretization points for 'min_max_range_parameter_0'
