...
def globalFreq(w, Kvco, AkPD, GkLF, Gvga, tauf, v, digital, maxp, inphase,INV):
	...
	if inphase:
		if digital=='True':
			...
			Omega = w + 0.5* K(Kvco, AkPD,  GkLF,Gvga) * np.sign(-p)*(1.0+ sawtooth( -p*np.sign(-p) ,width=1) )
			tau	  = ( Omega * tauf + v * p ) / Omega
	else:
		if digital:
...
def linStabEq(l_vec, Omega, tau, tauf, Kvco, AkPD,  GkLF,Gvga, tauc, v, zeta, order, digital, inphase, INV, filter):
	...
	if inphase:
		if digital=='True':
			...
			alpha = ( K(Kvco, AkPD, GkLF,Gvga)/(2.0*np.pi*v) )
...
def solveLinStabSinglePLL(Omega, tau, tauf, Kvco, AkPD,  GkLF,Gvga, tauc, v, order, digital, maxp, inphase, expansion, INV, filter):
