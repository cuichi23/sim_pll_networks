...
AkPD	= 1					# amplitude of the PD -- the factor of 2 here is related to the IMS 2020 paper, AkPD as defined in that paper already includes the 0.5 that we later account for when calculating K
...
if wnormx == w:
	...
	plt.xlabel(r'$\tau$', fontsize=18)
plt.ylabel(r'$\sigma$, $\gamma$', fontsize=18)


# calculate tau
tau = np.asarray(wnormx/(2.0*np.pi))*globalFreq(w, Kvco, AkPD, Ga1, tauf, v, digital, maxp, inphase1, INV)['tau']

# calculate stability
solved = solveLinStab(globalFreq(w, Kvco, AkPD, Ga1, tauf, v, digital, maxp, inphase1,INV)['Omeg'],
                      globalFreq(w,Kvco, AkPD, Ga1, tauf, v, digital, maxp, inphase1,INV)['tau'],
                      tauf, Kvco, AkPD, Ga1, tauc, v, order, digital, maxp, inphase1, expansion, INV)

# draw the initial plot
[lineSigmaIn] = ax1.plot(tau, solved['Re'],
	 linewidth=4, color='red', label=r'$\sigma$=Re$(\lambda)$ (in-phase)')

[lineGammaIn] = ax1.plot(tau, solved['Im'],
...
solved['tau'] = tau
# Export Data in numpy binary
filename = "K_" + str(int(Kvco*AkPD*Ga1)) + "_LF_" + str(tauc) + "_Order_" + str(order)
np.save("solved/" + filename, solved)
