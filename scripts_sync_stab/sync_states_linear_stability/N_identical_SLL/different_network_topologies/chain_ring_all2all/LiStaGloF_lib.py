...
def solveLinStab(Omega, tau, tauf, K, tauc, v, digital, maxp, expansion):
	lambsolRe = np.zeros(len(xrange(maxp, tauf)),dtype=np.complex)
	lambsolIm = np.zeros(len(xrange(maxp, tauf)),dtype=np.complex)
	init = initial_guess();
	zeta = -1;
	c 	 = 3E8;
...
def solveLinStab_topology_comparison(Omega, tau, tauf, K, tauc, v, digital, maxp, expansion, zetas):
	...
	lambsolRe 	 = np.zeros([len(zetas),len(xrange(maxp, tauf))],dtype=np.complex)
	lambsolIm 	 = np.zeros([len(zetas),len(xrange(maxp, tauf))],dtype=np.complex)
	lambsolReMax = np.zeros(len(xrange(maxp, tauf)),dtype=np.complex)
	lambsolImMax = np.zeros(len(xrange(maxp, tauf)),dtype=np.complex)
	init = initial_guess();
	#print(type(init), init)
	#print(len(xrange(K)))
