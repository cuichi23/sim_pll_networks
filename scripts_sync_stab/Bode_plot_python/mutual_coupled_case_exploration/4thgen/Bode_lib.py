...
def globalFreq(wref, w,  Kvco, AkPD, GkLF, Gvga, tauf, v, digital, maxp, sync_state):
	p = xrange(maxp, tauf)
	if sync_state == 'inphase':
		if digital:
			beta  = 0.0;
			Omega = w + K(Kvco, AkPD,  GkLF,Gvga)* cfDig( p )
			tau	  = ( Omega * tauf + v * p ) / Omega
			# np.where((tau.any()>0.2 and tau.any()<0.25), print('tauIn=', tau,'OmIn=', Omega))

		else:
			beta  = 0.0;
			Omega = w + K(Kvco, AkPD,  GkLF,Gvga) * cfAna( -p )
			tau	  = ( Omega * tauf + v * p ) / Omega
...
def linStabEq(l_vec, wref, w, Omega, tau, tauf, Kvco, AkPD,  GkLF,Gvga, tauc, v, zeta, digital, sync_state):
	x = np.zeros(2)
	if sync_state == 'inphase':
		if digital:
...
def solveLinStab(wref, w, Omega, tau, tauf, Kvco, AkPD, GkLF,Gvga, tauc, v, digital, maxp, sync_state, expansion):
	...
	if sync_state == 'inphase':
		if expansion:
			for index in range(len(xrange(maxp, tauf))):
				...
				# print(index, wR[index])
					temp =  optimize.root(linStabEq_expansion, (init[index2],init[index2]), args=(wref, w, Omega[index], tau[index], tauf, Kvco, AkPD, GkLF,Gvga, tauc, v, zeta, digital, sync_state), tol=1e-14, method='hybr')
				# print('temp =',temp)
				# print('temp =',temp)
			...
			for index in range(len(xrange(maxp, tauf))):
				for index2 in range(len(init)):
					# print(init[1])
					temp = optimize.root(linStabEq, (init[index2],init[index2]), args=(wref, w, Omega[index], tau[index], tauf, Kvco, AkPD, GkLF,Gvga,tauc, v, zeta, digital, sync_state),  tol=1.0e-14, method='hybr')
					lambsolRe[index2,index]  = temp.x[0]
					lambsolIm[index2,index]  = temp.x[1]
					# print(lambsolRe)
		...
		if expansion:
			...
			for index in range(len(xrange(maxp, tauf))):
				for index2 in range(len(init)):
					# print(init[1])
					temp = optimize.root(linStabEq, (init[index2],init[index2]), args=(wref, w, Omega[index], tau[index], tauf, K, tauc, v, zeta, digital, sync_state),  tol=1.0e-14, method='hybr')
					lambsolRe[index2,index]  = temp.x[0]
					lambsolIm[index2,index]  = temp.x[1]
					# print(lambsolRe)
					# lambsolRebetain[index]  = temp.x[0]
...
def LoopGainSteadyState(Omega, tau, tauf, v, Kvco, AkPD, GkLF, Gvga, digital):
	if digital:
		alpha = (K(Kvco, AkPD, GkLF, Gvga)/v)*cfDigDeriv(-Omega*(tau-tauf)/v)
	else:
		alpha = (K(Kvco, AkPD, GkLF, Gvga)/v)*cfAnaDeriv(-Omega*(tau-tauf)/v)
...
def HopenloopMutuallyCoupledOnePLL(w, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital):
	if digital:
		alpha = (K(Kvco, AkPD, GkLF, Gvga)/v)*cfDigDeriv(-Omega*(tau-tauf)/v)
	else:
		alpha = (K(Kvco, AkPD, GkLF, Gvga)/v)*cfAnaDeriv(-Omega*(tau-tauf)/v)

	return (alpha/(1j*w*(1+1j*w*tauc)))

def PhaseopenloopMutuallyCoupledOnePLL(w, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital):
	phase =	 cmath.phase(HopenloopMutuallyCoupledOnePLL(w, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital))
	return 360.0*phase/(2.0*np.pi)


def HclosedloopMutuallyCoupledOnePLL(w, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital):
	if digital:
		alpha = (K(Kvco, AkPD, GkLF, Gvga)/v)*cfDigDeriv(-Omega*(tau-tauf)/v)
	else:
		alpha = (K(Kvco, AkPD, GkLF, Gvga)/v)*cfAnaDeriv(-Omega*(tau-tauf)/v)

	return HopenloopMutuallyCoupledOnePLL(w, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital)/(1.0+HopenloopMutuallyCoupledOnePLL(w, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital)*np.exp(1j*w*(-tauf)))
	# return ((alpha*np.exp(1j*w*(-tauf))/(1j*w*(1+1j*w*tauc)))/(1.0+(alpha/(1j*w*(1+1j*w*tauc)))))


def PhaseclosedloopMutuallyCoupledOnePLL(w, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital):
	phase =	 cmath.phase(HclosedloopMutuallyCoupledOnePLL(w, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital))
	return 360.0*phase/(2.0*np.pi)


def HopenloopMutuallyCoupledNet(w, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital):
	if digital:
		alpha = (K(Kvco, AkPD, GkLF, Gvga)/v)*cfDigDeriv(-Omega*(tau-tauf)/v)
	else:
		alpha = (K(Kvco, AkPD, GkLF, Gvga)/v)*cfAnaDeriv(-Omega*(tau-tauf)/v)

	return (np.exp(-2*1j*w*tau)*(HclosedloopMutuallyCoupledOnePLL(w, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital))**2 )

	# return (np.exp(-2*1j*w*tau)*((alpha*np.exp(1j*w*(-tauf))/(1j*w*((1+1j*w*tauc)) ))/(1.0+(alpha*np.exp(1j*w*(-tauf))/(1j*w*((1+1j*w*tauc)) ))))**2)



def PhaseopenloopMutuallyCoupledNet(w, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital):
	phase =	 cmath.phase(HopenloopMutuallyCoupledNet(w, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital))
	return 360.0*phase/(2.0*np.pi)
...
def HclosedloopMutuallyCoupledNet(w, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital):
	if digital:
		alpha = (K(Kvco, AkPD, GkLF, Gvga)/v)*cfDigDeriv(-Omega*(tau-tauf)/v)
	else:
		alpha = (K(Kvco, AkPD, GkLF, Gvga)/v)*cfAnaDeriv(-Omega*(tau-tauf)/v)
...
def PhaseclosedloopMutuallyCoupledNet(w, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital):
	phase =	 cmath.phase(HopenloopMutuallyCoupledNet(w, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital))
	return 360.0*phase/(2.0*np.pi)
...
def GainMarginMutuallyCoupledNet(w, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital):
	if digital:
		alpha = (K(Kvco, AkPD, GkLF, Gvga)/v)*cfDigDeriv(-Omega*(tau-tauf)/v)
	else:
		alpha = (K(Kvco, AkPD, GkLF, Gvga)/v)*cfAnaDeriv(-Omega*(tau-tauf)/v)

	realpart=HopenloopMutuallyCoupledNet(w, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital).real
	imagpart=HopenloopMutuallyCoupledNet(w, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital).imag
	return np.sqrt(realpart**2+imagpart**2)





def PhaseMarginMutuallyCoupledNet(w, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital):
	if digital:
		alpha = (K(Kvco, AkPD, GkLF, Gvga)/v)*cfDigDeriv(-Omega*(tau-tauf)/v)
	else:
		alpha = (K(Kvco, AkPD, GkLF, Gvga)/v)*cfAnaDeriv(-Omega*(tau-tauf)/v)
...
#
# def Holnodel(w, wref, wvco, tau, tau_f, v, tau_c, K):
# 	return (K*np.sin(-(wref-wvco)/K))/((1.0+1j*w*tau_c)*(1j*w*v))
#
#
# def Hcl(w, wref, wvco, tau, tau_f, v, tau_c, K):
# 	beta 		= v*np.arccos((wref-wvco)/K)-wref*(tau-tau_f)
# 	alpha    	= K*np.sin(wref*(tau-tau_f)+beta);
# 	return alpha/(np.exp(1j*w*(tau-tau_f))*(v*np.exp(1j*w*tau_f)*(1j*w+tau_c*(1j*w)**2)+alpha))
#
#
# def Holwithdel(w, wref, wvco, tau, tau_f, v, tau_c, K):
# 	beta 		= v*np.arccos((wref-wvco)/K)-wref*(tau-tau_f)
# 	alpha    	= K*np.sin(wref*(tau-tau_f)+beta);
# 	# print(beta)
# 	# print(alpha)
#
# 	return alpha*np.exp(1j*w*(-tau-tau_f))/((1.0+1j*w*tau_c)*(1j*w*v))
#
#
#
# 	# alpha/((v*tau_c*(1j*w)**2+ v*(1j*w)+alpha*np.exp(tau_f*(1j*w)))*np.exp(1j*w*(tau-tau_f))
# def fMolnodel(w, wref, wvco, tau, tau_f, v, tau_c, K):
#
# 	 phase		=	cmath.phase(K/((1.0+1j*w*tau_c)*(1j*w*v)))
# 	 # print(phase)
# 	 return 360.0*phase/(2.0*np.pi)
#
# def fMcl(w, wref, wvco, tau, tau_f, v, tau_c, K):
# 	 beta		= v*np.arccos((wref-wvco)/K)-wref*(tau-tau_f)
# 	 alpha     	= 	K*np.sin((wref*(tau-tau_f))/v+beta);
# 	 # print(beta)
# 	 # print(alpha)
# 	 phase		=	cmath.phase(alpha/(np.exp(1j*w*(tau-tau_f))*(v*np.exp(1j*w*tau_f)*(1j*w+tau_c*(1j*w)**2)+alpha)))
#
# 	 return 360.0*phase/(2.0*np.pi)
#
#
# def fMolwithdel(w, wref, wvco, tau, tau_f, v, tau_c, K):
# 	 beta		= v*np.arccos((wref-wvco)/K)-wref*(tau-tau_f)
# 	 alpha     	= 	K*np.sin((wref*(tau-tau_f))/v+beta);
# 	 # print(beta)
# 	 # print(alpha)
# 	 phase		=	cmath.phase(alpha*np.exp(1j*w*(-tau-tau_f))/((1.0+1j*w*tau_c)*(1j*w*v)))
#
# 	 return 360.0*phase/(2.0*np.pi)
