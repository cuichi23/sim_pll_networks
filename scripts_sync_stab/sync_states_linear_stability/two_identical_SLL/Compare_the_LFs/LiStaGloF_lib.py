...
def linStabEq(l_vec, Omega, tau, tauf, K, tauc, v, zeta, order, digital):
	x = np.zeros(2)
	if digital:
	...
	f = l*((1.0+l*tauc)**order)*np.exp(l*tauf) + alpha*(1.0-zeta*np.exp(-l*(tau-tauf)))
	#print(f)
	x[0] = np.real(f)
	x[1] = np.imag(f)
...
def linStabEq_expansion(l_vec, Omega, tau, tauf, K, tauc, v, zeta,order, digital):
	x = np.zeros(2)
	if digital:
		alpha = (K/v)*cfDigDeriv(-Omega*(tau-tauf)/v)
	else:
...
def solveLinStab(Omega, tau, tauf, K, tauc, v,order, digital, maxp, expansion):
	...
	if expansion:
		for index in range(len(xrange(maxp, tauf))):
			...
			temp =  optimize.root(linStabEq_expansion, init, args=(Omega[index], tau[index], tauf, K, tauc, v, zeta, order,digital), tol=1e-14, method='hybr')
		# print('temp =',temp)
		# print('temp =',temp)
		...
		for index in range(len(xrange(maxp, tauf))):
			...
			temp = optimize.root(linStabEq, init, args=(Omega[index], tau[index], tauf, K, tauc, v, zeta, order, digital), tol=1e-14, method='hybr')
		# print('temp =',temp)
			lambsolRe[index] = temp.x[0]
			lambsolIm[index] = abs(temp.x[1])
		# print('temp.x[0] =', temp.x[0], 'temp.x[1] =', temp.x[1])
