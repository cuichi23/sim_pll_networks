...
import Intersection_lib
from Intersection_lib import integrationdigital
...
#choose the inphase or the antiphase synchronized state
inphase1 = True;
inphase2 = False;
...
# plt.ylabel(r'$\Omega/\bar{\omega}$', fontsize=18)
#
# [lineDigitalOmegainphase] 	  = ax3.plot(taudig, (solveglobalFreqdigital(wmean,  Kvco, AkPD, Ga1, tauf, v,  digital, maxp, Dw, inphase1)['Omega'])/wmean, '.',
# 									markersize=10, color='blue', label='Inphase')
...
[lineDigitalOmega] 	  = ax3.plot((wmean/(2.0*np.pi))*integrationdigital(wmean,Dw, Kvco, AkPD, Ga1,tauf, v)['Tau'][np.where(integrationdigital(wmean,Dw, Kvco, AkPD, Ga1, tauf, v)['OmegaDig']!=0)], integrationdigital(wmean,Dw, Kvco, AkPD, Ga1, tauf, v)['OmegaDig'],'.',
...
[lineDigitalbeta] 	  = ax4.plot((wmean/(2.0*np.pi))*integrationdigital(wmean,Dw, Kvco, AkPD, Ga1,tauf, v)['Tau'][np.where(integrationdigital(wmean,Dw, Kvco, AkPD, Ga1, tauf, v)['betadig']!=0)],(1.0/v)*integrationdigital(wmean,Dw, Kvco, AkPD, Ga1, tauf, v)['betadig'],'.',
									markersize=10, color='blue')
...
[lineDigitalbeta1] 	  = ax5.plot((wmean/(2.0*np.pi))*integrationdigital(wmean,Dw, Kvco, AkPD, Ga1,tauf, v)['Tau'][np.where(integrationdigital(wmean,Dw, Kvco, AkPD, Ga1, tauf, v)['betadig']!=0)],
									np.mod((integrationdigital(wmean,Dw, Kvco, AkPD, Ga1, tauf, v)['betadig']), 2.0*np.pi), '.',
									markersize=10, color='blue')
...
#
#
# [lineGammainphase] = ax5.plot((wmean/(2.0*np.pi))*globalFreq(wmean,  Kvco, AkPD, Ga1, tauf,v,  digital, maxp, Dw, inphase1)['taubetain'],
...
# #
#
# [lineGammaantiinphase] = ax5.plot((wmean/(2.0*np.pi))*globalFreq(wmean,  Kvco, AkPD, Ga1, tauf,v,  digital, maxp, Dw, inphase2)['taubetaanti'],#
