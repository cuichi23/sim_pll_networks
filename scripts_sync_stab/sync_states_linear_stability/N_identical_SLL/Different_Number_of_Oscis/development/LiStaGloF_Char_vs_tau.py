...
[lineSigmaSquare2d] = ax1.plot(np.asarray(w/(2.0*np.pi))*globalFreq(w, K, tauf, v, digital, maxp)['tau'], np.asarray((2.0*np.pi)/w)*solveLinStab_topology_comparison(globalFreq(w, K, tauf, v, digital, maxp)['Omeg'],
	globalFreq(w, K, tauf, v, digital, maxp)['tau'], tauf_0, K_0, tauc_0, v_0, digital,maxp,al, eigenvalzeta('square-periodic',Nx,Ny)['zet'])['ReMax'], '-', linewidth=1.5, color='blue', label=r'Re$(\lambda)$ ring')
...
[lineGammaSquare2d] = ax2.plot(np.asarray(w/(2.0*np.pi))*globalFreq(w, K, tauf, v, digital, maxp)['tau'], np.asarray(1/w)*solveLinStab_topology_comparison(globalFreq(w, K, tauf, v, digital, maxp)['Omeg'],
	globalFreq(w, K, tauf, v, digital, maxp)['tau'], tauf_0, K_0, tauc_0, v_0, digital,maxp,al, eigenvalzeta('square-periodic',Nx,Ny)['zet'])['ImMax'], '-', linewidth=1.5, color='blue', label=r'Im$(\lambda)$ 2d square')
...
def sliders_on_changed(val):
	...
	lineOmeg.set_ydata(solveLinStab_topology_comparison(globalFreq(w, K_slider.val, tauf_slider.val, v_slider.val, digital, maxp)['Omeg'],
						...
						tauf_slider.val, v_slider.val, digital,maxp, al, eigenvalzeta('global',Nx,Ny)['zet'])['Omeg']/np.asarray(w));
	lineOmeg.set_xdata(np.asarray(w/(2.0*np.pi))*solveLinStab_topology_comparison(globalFreq(w, K_slider.val, tauf_slider.val, v_slider.val, digital, maxp)['Omeg'],
						globalFreq(w, K_slider.val, tauf_slider.val, v_slider.val, digital, maxp)['tau'], tauf_slider.val, K_slider.val,
						tauf_slider.val, v_slider.val, digital,maxp, al, eigenvalzeta('global',Nx,Ny)['zet'])['tau']);
