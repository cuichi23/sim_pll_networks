...
def Knonlinear(K, AkPD, Ga1):
	return (K*Ga1*AkPD)/2.0
# set initial guess
# def initial_guess():
# 	return (3.101, 1.01);
def initial_guess():
	return (np.linspace(-0.701000001, 0.70100001, 10));
def initial_guess2():
	return (np.linspace(-0.601001810, 0.601001810, 10));
...
def globalFreqNonlinear(w, K, AkPD, Ga1, tauf, v, digital, maxp, inphase, INV):
	#Here we calculate analyticaly the Global Frequency of the synchronised states of the network of two SLLs.
	#First of all, there are two synchronised states, the in-phase and anti-phase. So, the inphase refers to that.
	#we distiguish the analog and the digital case of the SLLs. So, the digital refers to that.
	#The choice of digital and inphase mode is made in the main programs.
# 270000000.0*x + 825000000.0*sqrt(np.pi)*sps.erf(x - 1.07)
	...
	if inphase:
		if digital:
			Omega = w +  0.01*cfDig( p +INV)+sps.erf(Knonlinear(K, AkPD, Ga1)* cfDig( p +INV))
			tau	  = ( Omega * tauf + v * p ) / Omega
			# np.where((tau.any()>0.2 and tau.any()<0.25), print('tauIn=', tau,'OmIn=', Omega))

		else:
			Omega = w + 0.01*cfAna( p +INV)+sps.erf(Knonlinear(K, AkPD, Ga1) * cfAna( p +INV))
			tau	  = ( Omega * tauf + v * p ) / Omega
			# np.where((tau.any()>0.2 and tau.any()<0.25), print('tau=',tau,'Om=', Omega))
...
def linStabEqNonlinear(l_vec, Omega, tau, tauf, K, AkPD, Ga1, tauc, v, zeta, order, digital, inphase, INV):
	#This function gives the characteristic equation of the model
	#alpha = K/v*h'(Omega*(tau-tauf)/v)
	x = np.zeros(2)
	if inphase:
		if digital:
			alpha = np.exp(-(Knonlinear(K, AkPD, Ga1)* cfDig( -Omega*(tau-tauf)/v+INV))**2)*(1/v)*cfDigDeriv(-Omega*(tau-tauf)/v+INV)
		else:
			alpha =np.exp(-(Knonlinear(K, AkPD, Ga1)* cfAna( -Omega*(tau-tauf)/v +INV))**2)*(1/v)*cfAnaDeriv(Omega*(tau-tauf)/v+INV)
...
def solveLinStab(Omega, tau, tauf, Kvco, AkPD, Ga1, tauc, v, order, digital, maxp, inphase, expansion, INV, zeta):
...
def solveLinStabNonliner(Omega, tau, tauf, K, AkPD, Ga1, tauc, v, order, digital, maxp, inphase, expansion, INV, zeta):
...
##########################################################
	...
	for index in range(len(xrange(maxp, tauf))):
		for index2 in range(len(init)):
			# print(init[1])
			temp = optimize.root(linStabEqNonlinear, (init[index2],init2[index2]), args=( Omega[index], tau[index], tauf, K, AkPD, Ga1, tauc, v, zeta, order, digital, inphase, INV),  tol=1.0e-14, method='hybr')
			lambsolRe[index2,index]  = temp.x[0]
			lambsolIm[index2,index]  = temp.x[1]
		tempRe = lambsolRe[np.round(lambsolRe[:,index],16)!=0.0,index];
		tempIm = lambsolIm[np.round(lambsolIm[:,index],16)!=0.0,index];
