...
dict_pll={
	...
	'transmission_delay': 0.85, 												# value of transmission delay in seconds, float (single), list (tau_k) or list of lists (tau_kl): np.random.uniform(min,max,size=[dict_net['Nx']*dict_net['Ny'],dict_net['Nx']*dict_net['Ny']]), OR [np.random.uniform(min,max) for i in range(dict_net['Nx']*dict_net['Ny'])]
...
for i in range(len(K)):
	dict_pll.update({'coupK': K[i]/(2*np.pi)})									# set this temporarly to one value -- in Hz
	for j in range(len(wc)):
		...
		if len(para_mat[:,4]) > 1:
			...
			#print('Found one synchronized state, Omega:', para_mat[:,4], '\tfor (alpha, tau, beta)=(', dict_pll['coupK'], dict_pll['transmission_delay'], beta,').')
