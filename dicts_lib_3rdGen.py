...
def getDicts(Fsim=100):
	...
	dictNet={
		...
		'Tsim': 135000,															# simulation time in multiples of the period
	...
	dictPLL={
		...
		'transmission_delay': 2.99,#3.05,#1571.805,								# value of transmission delay in seconds, float (single), list (tau_k) or list of lists (tau_kl): np.random.uniform(min,max,size=[dictNet['Nx']*dictNet['Ny'],dictNet['Nx']*dictNet['Ny']]), OR [np.random.uniform(min,max) for i in range(dictNet['Nx']*dictNet['Ny'])]
	...
	dictAlgo={
		...
		'paramDiscretization': [5, 1],#[15, 10],								# parameter discetization for brute force parameter space scans
