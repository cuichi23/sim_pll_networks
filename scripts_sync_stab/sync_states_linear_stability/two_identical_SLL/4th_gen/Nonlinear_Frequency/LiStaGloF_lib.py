...
def NonlinearFreqResponse(AkPD, GkLF, Gvga, Dphi, Vcoarse, INV,digital):
	#f=a(Vcoarse)+b(Vcoarse)*sps.erf(Vfine-VPD/2)
	a1=6.23345e10
	a2=1.45319e9
	a3=1.5
	b1=1.53427e8

	a=a1+a2*sps.erf(Vcoarse-a3)
	b=b1*Vcoarse

	VPD=2.7
	# print(a)
	if digital:
		Vfine=AkPD*GkLF*Gvga*cfDig( Dphi + INV )
		return 2.0*np.pi*( a+b*sps.erf(Vfine-VPD/2))
		# return 300000000.0*cfDig( Dphi + INV ) + 825000000.0*np.sqrt(np.pi)*sps.erf(cfDig( Dphi + INV ) - 1.032)+7.8e8
	else:
		Vfine=AkPD*GkLF*Gvga*cfAna( Dphi + INV )
		# print(Vfine)
		# print(cfAna( Dphi + INV ))
		return 2.0*np.pi*( a+b*sps.erf(Vfine-VPD/2))

def NonlinearKvco(Omega, tau, tauf, v, AkPD, GkLF, Gvga, Vcoarse, INV, digital):
	b1=1.5224e8
	b=b1*Vcoarse


	if digital:
		return 2.0*np.pi*(2.0*b/np.sqrt(np.pi))*cfDigDeriv( -Omega*( tau-tauf )/v  + INV )*np.exp(-(1.35 - AkPD*GkLF*Gvga*cfDig(  -Omega*(tau-tauf)/v +INV  ))**2)
	else:
		return 2.0*np.pi*(2.0*b/np.sqrt(np.pi))*cfAnaDeriv( -Omega*( tau-tauf )/v  + INV )*np.exp(-(1.35 - AkPD*GkLF*Gvga*cfAna(  -Omega*(tau-tauf)/v +INV  ))**2)
...
def globalFreqNonlinear(AkPD, GkLF, Gvga, Vcoarse, tauf, v, digital, maxp, inphase, INV):
	#Here we calculate analyticaly the Global Frequency of the synchronised states of the network of two SLLs.
	#First of all, there are two synchronised states, the in-phase and anti-phase. So, the inphase refers to that.
	#we distiguish the analog and the digital case of the SLLs. So, the digital refers to that.
	#The choice of digital and inphase mode is made in the main programs.
# 270000000.0*x + 825000000.0*sqrt(np.pi)*sps.erf(x - 1.07)
	...
	if inphase:
		if digital:
			Omega = NonlinearFreqResponse(AkPD, GkLF, Gvga, Dphi, Vcoarse, INV,digital)
			tau	  = ( Omega * tauf + v * p ) / Omega
			# np.where((tau.any()>0.2 and tau.any()<0.25), print('tauIn=', tau,'OmIn=', Omega))
...
def linStabEqNonlinear(l_vec, Omega, tau, tauf, tauc, AkPD, GkLF, Gvga, Vcoarse, v, zeta, order, digital, inphase, INV):
	#This function gives the characteristic equation of the model
	#alpha = K/v*h'(Omega*(tau-tauf)/v)
	x = np.zeros(2)
	if inphase:
		if digital:
			alpha = NonlinearKvco(Omega, tau, tauf, v, AkPD, GkLF, Gvga, Vcoarse, INV, digita)*(1/v)#*cfDigDeriv(-Omega*(tau-tauf)/v+INV)
		else:
			alpha = NonlinearKvco(Omega, tau, tauf, v, AkPD, GkLF, Gvga, Vcoarse, INV, digital)*(1/v)#*cfAnaDeriv(-Omega*(tau-tauf)/v+INV)
...
def solveLinStabNonliner(Omega, tau, tauf, tauc, AkPD, GkLF, Gvga, Vcoarse, v, order, digital, maxp, inphase, expansion, INV, zeta):
...
##########################################################
	...
	for index in range(len(xrange(maxp, tauf))):
		for index2 in range(len(init)):
			# print(init[1])
			temp = optimize.root(linStabEqNonlinear, (init[index2],init[index2]), args=( Omega[index], tau[index], tauf, tauc,AkPD, GkLF, Gvga, Vcoarse, v, zeta, order, digital, inphase, INV),  tol=1.0e-14, method='hybr')
