...
def xrange(maxp, tauf):
	dp		= 0.015;
	initial=1490.0
	return np.linspace(initial, maxp, (-initial+maxp)/dp)
...
def linStabEq_expansion(l_vec, Omega, tau, tauf, Kvco, AkPD,  GkLF,Gvga, tauc, v, zeta, digital, INV):
	x = np.zeros(2)
	if digital:
		alpha = (K(Kvco, AkPD, GkLF,Gvga)/v)*cfDigDeriv(-Omega*(tau-tauf)/v+INV)
	else:
...
def solveLinStab(Omega, tau, tauf, Kvco, AkPD,  GkLF,Gvga, tauc, v, digital, maxp, expansion, INV):
	...
	if expansion:
		for index in range(len(xrange(maxp, tauf))):
			...
			temp = optimize.root(linStabEq_expansion, init, args=(Omega[index], tau[index], tauf,Kvco, AkPD,  GkLF,Gvga, tauc, v, zeta, digital, INV), tol=1e-14, method='hybr')
		# print('temp =',temp)
		...
		for index in range(len(xrange(maxp, tauf))):
			...
			temp = optimize.root(linStabEq, init, args=(Omega[index], tau[index], tauf, Kvco, AkPD,  GkLF,Gvga, tauc, v, zeta, digital, INV), tol=1e-14, method='hybr')
		# print('temp =',temp)
			lambsolRe[index] = temp.x[0]
			lambsolIm[index] = abs(temp.x[1])
		# print('temp.x[0] =', temp.x[0], 'temp.x[1] =', temp.x[1])
...
# 	l = l_vec[0] + 1j * l_vec[1]
#
# 	return {l*(1.0+l*tauc)*np.exp(l*tauf)+tauf*(1.0+l*tauc)*np.exp(l*tauf) - alpha*(tau-tauf)*zeta*np.exp(-l*(tau-tauf))}


def solveLinStab_topology_comparison(Omega, tau, tauf, Kvco, AkPD,  GkLF, Gvga, tauc, v, digital, maxp, expansion, zetas, INV):
	# global x;
	...
	if expansion:
		for index1 in range(len(xrange(maxp, tauf))):
			for index2 in range(len(zetas)):
				# print(index, wR[index])
				temp = optimize.root(linStabEq_expansion, init, args=(Omega[index1], tau[index1], tauf, Kvco, AkPD,  GkLF,Gvga, tauc, v, zetas[index2], digital, INV), tol=1e-14, method='hybr')
				# print('temp =',temp)
		...
		for index1 in range(len(xrange(maxp, tauf))):
			for index2 in range(len(zetas)):
				# print(index, wR[index])
				temp = optimize.root(linStabEq, init, args=(Omega[index1], tau[index1], tauf, Kvco, AkPD,  GkLF,Gvga, tauc, v, zetas[index2], digital, INV), tol=1.49012e-08,  method='hybr')
