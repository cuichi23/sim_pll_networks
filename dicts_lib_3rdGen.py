...
def getDicts(Fsim=55):
	...
	dictNet={
		...
		'Tsim': 325000,															# simulation time in multiples of the period
		...
		'phiInitConfig': [0, np.pi/2],												# phase-configuration of sync state,  []: automatic, else provide list
	...
	dictPLL={
		'intrF': 1.0,#[0.999938, 1.000061],											# intrinsic frequency in Hz [random.uniform(0.95, 1.05) for i in range(dictNet['Nx']*dictNet['Ny'])]
		'syncF': 1.0,															# frequency of synchronized state in Hz
		'coupK': 0.0500991,														# [random.uniform(0.3, 0.4) for i in range(dictNet['Nx']*dictNet['Ny'])],# coupling strength in Hz float or [random.uniform(minK, maxK) for i in range(dictNet['Nx']*dictNet['Ny'])]
		...
		'cutFc': 16.5E-6,														# LF cut-off frequency in Hz, here N=9 with mean 0.015: [0.05,0.015,0.00145,0.001,0.0001,0.001,0.00145,0.015,0.05]
		...
		'transmission_delay': 1571,												# value of transmission delay in seconds, float (single), list (tau_k) or list of lists (tau_kl): np.random.uniform(min,max,size=[dictNet['Nx']*dictNet['Ny'],dictNet['Nx']*dictNet['Ny']]), OR [np.random.uniform(min,max) for i in range(dictNet['Nx']*dictNet['Ny'])]
	...
	dictAlgo={
		'bruteForceBasinStabMethod': 'single', #'listOfInitialPhaseConfigurations',		# pick method for setting realizations 'single', 'classicBruteForceMethodRotatedSpace', 'listOfInitialPhaseConfigurations', 'single', 'statistics'
		'paramDiscretization': [3, 2],#[15, 10],								# parameter discetization for brute force parameter space scans
