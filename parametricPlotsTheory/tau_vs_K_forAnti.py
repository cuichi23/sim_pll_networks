...
dict_net={
	...
	'my': 1,																	# twist/chequerboard in y-direction
	'Tsim': 100,
	...
	'zeta': [-1, -0.5, 0.5],													# real part of eigenvalue of slowest decaying perturbation mode for the set of parameters, also a fct. of tau!
	'psi': [np.pi, np.pi, 0.0],													# real part of eigenvalue of slowest decaying perturbation
...
for i in range(len(tau)):
	...
	for j in range(len(K)):
		...
		if len(para_mat[:,4]) > 1:
			...
			# Kcrit[i,j] = ( 0.5 /( para_mat[index,3]*( dict_pll['derivative_coup_fct']( (-2.0*np.pi*para_mat[index,4]*para_mat[index,3]+beta)/para_mat[index,12] ) )))
			...
			# Kcrit[i,j]=(0.55/( para_mat[:,3]*(dict_pll['derivative_coup_fct']( (-2.0*np.pi*para_mat[:,4]*para_mat[:,3]+beta)/para_mat[:,12] ))))
