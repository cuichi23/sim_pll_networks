...
def getDicts(Fsim=125):
	...
	dictNet={
		...
		'Tsim': 850,															# simulation time in multiples of the period
		'phi_array_mult_tau': 1,												# how many multiples of the delay is stored of the phi time series
		'phiPerturb': [np.random.uniform(-0.01,0.01) for i in range(20)],		# delta-perturbation on initial state -- PROVIDE EITHER ONE OF THEM! if [] set to zero
	...
	dictPLL={
		...
		'gPDin': np.random.uniform(-1,1,size=[dictNet['Nx']*dictNet['Ny'],dictNet['Nx']*dictNet['Ny']]),# gains of the different inputs to PD k from input l -- G_kl, see PD, set to 1 and all G_kl=1 (so far only implemented for some cases, check!): np.random.uniform(0.95,1.05,size=[dictNet['Nx']*dictNet['Ny'],dictNet['Nx']*dictNet['Ny']])
		'gPDin_symmetric': True,												# set to True if G_kl == G_lk, False otherwise
		'cutFc': 0.4,															# LF cut-off frequency in Hz, here N=9 with mean 0.015: [0.05,0.015,0.00145,0.001,0.0001,0.001,0.00145,0.015,0.05]
