...
order	= 1.0					# the order of the Loop Filter
tauf    = 0.0					# tauf = sum of all processing delays in the feedback
tauc	= 1.0/(2.0*np.pi*15.6*1E-3);  # the integration time of the Loop Filter tauc=1/wc=1/(2πfc), fc the cutoff frequency of the Loop Filter
v		= 1					# the division
...
for j in range(len(globalFreq(w1, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['tau'])):
	for i in range(len(f)):
		...
		tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital)))<=0.05 and globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['tau'][j]!=0.0:# and np.real(HopenloopMutuallyCoupledNet(f[i]*2.0*np.pi,
		# globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['Omeg'][j],
		# globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['tau'][j],
		# tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital) ) >1.0025 and globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['tau'][j]!=0.0:
			openlooptr.append(1-np.real(HopenloopMutuallyCoupledNet(f[i]*2.0*np.pi,
			globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['Omeg'][j],
			globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['tau'][j],
...
openlooptr=np.asarray(openlooptr)
print(openlooptr)
			#
# for j in range(len(globalFreq(w1, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state2, INV)['tau'])):
# 	for i in range(len(f)):
...
#
#
fig5= plt.figure(num=3, figsize=(figwidth, figheight), dpi=dpi_val, facecolor='w', edgecolor='k')
# # # fig2, (, ) = plt.subplots(2)
# #
# #
ax6= plt.subplot(111)
...
#
# # 				#
# # [linecl05] 	=	ax6.plot(np.asarray(globalFreq(w1, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['Omeg']/(2.0*np.pi))*globalFreq(w1, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['tau'],
# # 					np.sqrt( (1.0-np.real(HopenloopMutuallyCoupledNet(2.0*np.pi*f, globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['Omeg'],
# # 					globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['tau'],
# # 					tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital)) )**2 +( np.imag(HopenloopMutuallyCoupledNet(2.0*np.pi*f,
# # 					globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['Omeg'],
# # 					globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['tau'],
# # 					tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital)) )**2 ),
# # 					linewidth=4,  label=r'Inphase distance ')
#
# # plt.grid();
# # plt.legend();
# # #
...
# [lineReHeIn] = ax6.plot( np.asarray(1/(2.0*np.pi))*results1[:,2]*results1[:,1],
# 		np.asarray(2.0*np.pi/w1)*results1[:,3], '.',ms=1, color='blue', label=r'Inphase Stable $Re$(H)$')
[l1inecl06] 	=	ax6.plot(np.asarray(taugain)*np.asarray(omegagain), openlooptr, '.',  label=r'Antiphase $1-Re(H(j\omega))$' )
#
...
[li1] 	=	ax1.plot(np.real(HopenloopMutuallyCoupledNet(2.0*np.pi*f, globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['Omeg'][0],
...
[li2] 	=	ax2.plot(np.real(HopenloopMutuallyCoupledNet(2.0*np.pi*f, globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['Omeg'][8],
...
[li3] 	=	ax3.plot(np.real(HopenloopMutuallyCoupledNet(2.0*np.pi*f, globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['Omeg'][9],
...
[li4] 	=	ax4.plot(np.real(HopenloopMutuallyCoupledNet(2.0*np.pi*f, globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['Omeg'][15],
...
[linl1] =	ax1.plot(np.real(HopenloopMutuallyCoupledNet(-2.0*np.pi*f, globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['Omeg'][0],
				globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['tau'][0], tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital)),
				np.imag(HopenloopMutuallyCoupledNet(-2.0*np.pi*f, globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['Omeg'][0],
				globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['tau'][0], tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital)),
				'--',ms=1, color= 'blue')
[linl2] =	ax2.plot(np.real(HopenloopMutuallyCoupledNet(-2.0*np.pi*f, globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['Omeg'][8],
				globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['tau'][8], tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital)),
				np.imag(HopenloopMutuallyCoupledNet(-2.0*np.pi*f, globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['Omeg'][8],
				globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['tau'][8], tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital)),
				'--',ms=1, color= 'green')
[linl3] =	ax3.plot(np.real(HopenloopMutuallyCoupledNet(-2.0*np.pi*f, globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['Omeg'][9],
				globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['tau'][9], tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital)),
				np.imag(HopenloopMutuallyCoupledNet(-2.0*np.pi*f, globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['Omeg'][9],
				globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['tau'][9], tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital)),
				'--',ms=1, color= 'orange')
[linl4] =	ax4.plot(np.real(HopenloopMutuallyCoupledNet(-2.0*np.pi*f, globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['Omeg'][15],
				globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['tau'][15], tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital)),
				np.imag(HopenloopMutuallyCoupledNet(-2.0*np.pi*f, globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['Omeg'][15],
				globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['tau'][15], tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital)),
				'--',ms=1, color= 'purple')
...
for i in range(len(bar)):
	...
	[linecl11] 	=	ax9.plot(np.real(HopenloopMutuallyCoupledNet(2.0*np.pi*f, globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['Omeg'][bar[i]],
					...
					globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['tau'][bar[i]], tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital)),
					'-',ms=1, color= colorbar[i], label=r'τ=%.5f' %(globalFreq(wref, w1, Kvco, AkPD, GkLF,Gvga, tauf, v, digital, maxp, sync_state1, INV)['tau'][bar[i]]))
