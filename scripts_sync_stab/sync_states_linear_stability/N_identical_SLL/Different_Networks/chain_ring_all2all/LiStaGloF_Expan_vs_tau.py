...
import topology
from topology import eigenvalzeta# from mpmath import findroot
...
# [lineSigmaSquare2d] = ax1.plot(np.asarray(w/(2.0*np.pi))*globalFreq(w, K, tauf, v, digital, maxp)['tau'], np.asarray((2.0*np.pi)/w)*solveLinStab_topology_comparison(globalFreq(w, K, tauf, v, digital, maxp)['Omeg'],
 	 # globalFreq(w, K, tauf, v, digital, maxp)['tau'], tauf_0, K_0, tauc_0, v_0, digital, maxp, expansion, eigenvalzeta('square-open',Nx,Ny)['zeta'])['ReMax'], '--', linewidth=1.5, color='blue', label=r'Re$(\lambda)$ square open')
#
[lineSigmaring]     = ax1.plot(np.asarray(w/(2.0*np.pi))*globalFreq(w, K, tauf, v, digital, maxp)['tau'], np.asarray((2.0*np.pi)/w)*solveLinStab_topology_comparison(globalFreq(w, K, tauf, v, digital, maxp)['Omeg'],
 	globalFreq(w, K, tauf, v, digital, maxp)['tau'], tauf_0, K_0, tauc_0, v_0, digital, maxp, expansion, eigenvalzeta('ring',Nx,Ny)['zeta'])['ReMax'], '-', linewidth=1.5, color='black', label=r'Re$(\lambda)$ ring')

[lineSigmachain]     = ax1.plot(np.asarray(w/(2.0*np.pi))*globalFreq(w, K, tauf, v, digital, maxp)['tau'], np.asarray((2.0*np.pi)/w)*solveLinStab_topology_comparison(globalFreq(w, K, tauf, v, digital, maxp)['Omeg'],
	globalFreq(w, K, tauf, v, digital, maxp)['tau'], tauf_0, K_0, tauc_0, v_0, digital, maxp, expansion, eigenvalzeta('chain',Nx,Ny)['zeta'])['ReMax'], '--', linewidth=1.5, color='green', label=r'Re$(\lambda)$ chain')
...
[lineGammaring] = ax2.plot(np.asarray(w/(2.0*np.pi))*globalFreq(w, K, tauf, v, digital, maxp)['tau'], np.asarray(1/w)*solveLinStab_topology_comparison(globalFreq(w, K, tauf, v, digital, maxp)['Omeg'],
	   globalFreq(w, K, tauf, v, digital, maxp)['tau'], tauf_0, K_0, tauc_0, v_0, digital,maxp, expansion, eigenvalzeta('ring',Nx,Ny)['zeta'])['ImMax'], '-', linewidth=1.5, color='black', label=r'Im$(\lambda)$ ring')

[lineGammachain] = ax2.plot(np.asarray(w/(2.0*np.pi))*globalFreq(w, K, tauf, v, digital, maxp)['tau'], np.asarray(1/w)*solveLinStab_topology_comparison(globalFreq(w, K, tauf, v, digital, maxp)['Omeg'],
	   globalFreq(w, K, tauf, v, digital, maxp)['tau'], tauf_0, K_0, tauc_0, v_0, digital,maxp, expansion, eigenvalzeta('chain',Nx,Ny)['zeta'])['ImMax'], '--', linewidth=1.5, color='green', label=r'Im$(\lambda)$ chain')
...
def sliders_on_changed(val):
	...
	lineOmeg.set_ydata(solveLinStab_topology_comparison(globalFreq(w, K_slider.val, tauf_slider.val, v_slider.val, digital, maxp)['Omeg'],
						...
						tauc_slider.val, v_slider.val, digital,maxp, expansion, eigenvalzeta('chain',Nx,Ny)['zeta'])['Omeg']/np.asarray(w));
	lineOmeg.set_xdata(np.asarray(w/(2.0*np.pi))*solveLinStab_topology_comparison(globalFreq(w, K_slider.val, tauf_slider.val, v_slider.val, digital, maxp)['Omeg'],
						globalFreq(w, K_slider.val, tauf_slider.val, v_slider.val, digital, maxp)['tau'], tauf_slider.val, K_slider.val,
						tauf_slider.val, v_slider.val, digital,maxp, expansion, eigenvalzeta('chain',Nx,Ny)['zeta'])['tau']);
