...
def xrange(maxp, tauf, dp=0.01): #dp=0.0005
...
def initial_guess():
	return (np.linspace( 1e-3, 1e-2, 10));

def initial_guess2():
	return (np.linspace(-0.48*2.0*np.pi, 0.50*2.0*np.pi, 10));
# digital case
	...
	if sync_state == 'inphase':
		if expansion:
			for index in range(len(xrange(maxp, tauf))):
				...
				# print(index, wR[index])
					temp =  optimize.root(linStabEq_expansion, (init[index2],init2[index2]), args=(wref, w, Omega[index], tau[index], tauf, Kvco, AkPD, GkLF,Gvga, tauc, v, zeta, digital, sync_state, INV ), tol=1e-14, method='hybr')
				# print('temp =',temp)
				# print('temp =',temp)
			...
			for index in range(len(xrange(maxp, tauf))):
				for index2 in range(len(init)):
					# print(init[1])
					temp = optimize.root(linStabEq, (init[index2],init2[index2]), args=(wref, w, Omega[index], tau[index], tauf, Kvco, AkPD, GkLF,Gvga,tauc, v, zeta, digital, sync_state, INV ),  tol=1.0e-14, method='hybr')
					lambsolRe[index2,index]  = temp.x[0]
					lambsolIm[index2,index]  = temp.x[1]
					# print(lambsolRe)
		...
		# # else:
			...
			for index in range(len(globalFreq(wref, Omega, K, tauf, v, digital, maxp, sync_state)['tau'])):
				for index2 in range(len(init)):
					# print(init[1])
					temp = optimize.root(linStabEq, (init[index2],init2[index2]), args=(wref, w, Omega, tau[index], tauf, Kvco, AkPD, GkLF,Gvga,tauc, v, zeta, digital, sync_state),  tol=1.0e-15, method='hybr')
					lambsolRe[index2,index]  = temp.x[0]
					lambsolIm[index2,index]  = temp.x[1]
					# print(lambsolRe)
					# lambsolRebetain[index]  = temp.x[0]
...
def LoopGainSteadyState(Omega, tau, tauf, v, Kvco, AkPD, GkLF, Gvga, digital, model, sync_state):
	if model=='Nonlinear':
		if sync_state == 'inphase':
			if digital:
				alpha = (K(Kvco, AkPD, GkLF,Gvga)/v)*cfDigDeriv(-Omega*(tau-tauf)/v)
...
def HopenLoopMutuallyCoupledOnePLL(gamma, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital, model, sync_state):
	return ( LoopGainSteadyState(Omega, tau, tauf, v, Kvco, AkPD, GkLF, Gvga, digital, model, sync_state)/( 1j*gamma*( 1.0 + 1j*gamma*tauc ) ) )

def PhaseopenloopMutuallyCoupledOnePLL(gamma, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital, model, sync_state):
	phase =	 cmath.phase(HopenLoopMutuallyCoupledOnePLL(gamma, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga,  digital, model, sync_state))
	return 360.0*phase/(2.0*np.pi)

def HclosedLoopMutuallyCoupledOnePLL(gamma, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital, model, sync_state):
	# return HopenLoopMutuallyCoupledOnePLL(gamma, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital, model, sync_state)/( 1.0 + HopenLoopMutuallyCoupledOnePLL(gamma, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital, model, sync_state)*np.exp(1j*gamma*( -tauf ) ) )
	return 1.0 / ( 1.0/HopenLoopMutuallyCoupledOnePLL(gamma, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga, digital, model, sync_state) + np.exp(1j*gamma*( -tauf ) ) )
	# return ((alpha*np.exp(1j*w*(-tauf))/(1j*w*(1+1j*w*tauc)))/(1.0+(alpha/(1j*w*(1+1j*w*tauc)))))

def PhaseclosedloopMutuallyCoupledOnePLL(gamma, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga,  digital, model, sync_state):
	phase =	 cmath.phase(HclosedLoopMutuallyCoupledOnePLL(gamma, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga,  digital, model, sync_state))
	return 360.0*phase/(2.0*np.pi)

def HopenLoopMutuallyCoupledNet(gamma, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga,  digital, model, sync_state):
	return (np.exp(-2.0*1j*gamma*tau)*(HclosedLoopMutuallyCoupledOnePLL(gamma, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga,  digital, model, sync_state))**2 )

	# return (np.exp(-2*1j*w*tau)*((alpha*np.exp(1j*w*(-tauf))/(1j*w*((1+1j*w*tauc)) ))/(1.0+(alpha*np.exp(1j*w*(-tauf))/(1j*w*((1+1j*w*tauc)) ))))**2)

def HopenLoopMutuallyCoupledNet2(gamma, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga,  digital, model, sync_state):
...
def PhaseopenloopMutuallyCoupledNet(gamma, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga,  digital, model, sync_state):
	phase =	 cmath.phase(HopenLoopMutuallyCoupledNet(gamma, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga,  digital, model, sync_state))
	return 360.0*phase/(2.0*np.pi)

def HclosedloopMutuallyCoupledNet(gamma, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga,  digital, model, sync_state):
	...
	return 1.0 / ( 1.0/HopenLoopMutuallyCoupledNet(gamma, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga,  digital, model, sync_state) - 1.0)

def PhaseclosedloopMutuallyCoupledNet(gamma, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga,  digital, model, sync_state):
	phase =	 cmath.phase(HopenLoopMutuallyCoupledNet(gamma, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga,  digital, model, sync_state))
	return 360.0*phase/(2.0*np.pi)

def GainMarginMutuallyCoupledNet(gamma, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga,  digital, model, sync_state):

	temp = HopenLoopMutuallyCoupledNet(gamma, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga,  digital, model, sync_state)
	realpart = temp.real
	imagpart = temp.imag
	return 1.0-np.abs(np.sqrt(realpart**2+imagpart**2))

def PhaseMarginMutuallyCoupledNet(gamma, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga,  digital, model, sync_state):
...
def GainMarginMutuallyCoupledOne(gamma, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga,  digital, model, sync_state):

	temp = HopenLoopMutuallyCoupledOnePLL(gamma, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga,  digital, model, sync_state)
	realpart = temp.real
	imagpart = temp.imag
	# print(realpart,imagpart)
	return 1.0-np.abs(np.sqrt(realpart**2+imagpart**2))

def PhaseMarginMutuallyCoupledOne(gamma, Omega, tau, tauf, tauc, v, Kvco, AkPD, GkLF, Gvga,  digital, model, sync_state):
