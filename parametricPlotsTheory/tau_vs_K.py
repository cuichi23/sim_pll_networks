...
K		= 2.0*np.pi*np.arange(0.001, 0.8, 0.06285/(2.0*np.pi) )
...
for i in range(len(tau)):
	...
	for j in range(len(K)):
		...
		if len(para_mat[:,4]) > 1:
			...
			#index = np.where(para_mat[:,4]==np.max(para_mat[:,4]))
			index = np.argmax(para_mat[:,4], axis=0)
			#print('index:', index, '\ttype(index)', type(index), '\tpara_mat[index,4]', para_mat[index,4].tolist())
...
paraPlot.plotParametric(params)
