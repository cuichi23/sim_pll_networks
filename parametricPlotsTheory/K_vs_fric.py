...
for i in range(len(K)):
	...
	for j in range(len(fric)):
		...
		if wc*fric[j]**2/(2*alpha[i,j]) < fzeta and wc*fric[j]**2/(2*alpha[i,j]) > 1:
			CondStab[i,j] = 1
		else:
			CondStab[i,j] = None
...
paraPlot.makePlotsFromSynctoolsResults(100, K, fric, OmegInKvsFc, 1.0/w, 1, 1.0,
				r'$\frac{K}{\omega}$', r'$\gamma$', r'$\Omega$', 'K', 'fric', 'Omeg', None, cm.coolwarm)
paraPlot.makePlotsFromSynctoolsResults(101, K, fric, ReLambda, 1.0/w, 1, w/(2.0*np.pi),
				r'$\frac{K}{\omega}$', r'$\gamma$', r'$\frac{\textrm{Re}(\lambda)\omega}{2\pi}$', 'K', 'fric', 'ReLambda', None, cm.PuOr)
paraPlot.makePlotsFromSynctoolsResults(102, K, fric, ImLambda, 1.0/w, 1, 1.0/w,
