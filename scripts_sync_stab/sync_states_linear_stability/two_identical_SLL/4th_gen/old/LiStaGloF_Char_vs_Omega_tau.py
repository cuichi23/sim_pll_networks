...
if digital == True:
	...
	plt.title(r'analog case for $\omega=$%0.2f GHz and Kvco=%0.2f' %(w/(2.0E9*np.pi), Kvco/(2.0E9*np.pi)) );
...
[lineSigma1] = ax1.plot(np.asarray(w/(2.0*np.pi))*globalFreq(w, Kvco, AkPD, Ga1, tauf, v, digital, maxp)['Omeg']*globalFreq(w, Kvco, AkPD, Ga1, tauf, v, digital, maxp)['tau'], np.asarray(2.0*np.pi/w)*solveLinStab(globalFreq(w, Kvco, AkPD, Ga1, tauf, v, digital, maxp)['Omeg'],
...
[lineGamma1] = ax1.plot(np.asarray(w/(2.0*np.pi))*globalFreq(w,Kvco, AkPD, Ga1, tauf, v, digital, maxp)['Omeg']*globalFreq(w, Kvco, AkPD, Ga1, tauf, v, digital, maxp)['tau'], np.asarray(1/w)*solveLinStab(globalFreq(w, Kvco, AkPD, Ga1, tauf, v, digital, maxp)['Omeg'],
...
v_slider      = Slider(v_slider_ax, r'$v$', 4, 8, valinit=v_0, valfmt='%1.2f', valstep=4)
# Draw another slider
...
tauc_slider     = Slider(tauc_slider_ax, r'$\tau^c$', 0.05*1.0/(2.0*np.pi*120E6), 1.0/(2.0*np.pi*120E6), valinit=tauc_0, valfmt='%1.2E')
