...
def K():
	return np.linspace(0.001, 1.0, 10)
...
def globalFreq(w, tauf, v, digital, maxp):
	p = xrange(maxp, tauf)
	if digital:
		Omega = w + K() * cfDig( -p )
...
def linStabEq(l_vec, Omega, tau, tauf,tauc, v, zeta, digital):
	x = np.zeros(2)
	if digital:
...
def linStabEq_expansion(l_vec, Omega, tau, tauf, tauc, v, zeta, digital):
	x = np.zeros(2)
	if digital:
		alpha = (K()/v)*cfDigDeriv(-Omega*(tau-tauf)/v)
	else:
...
def solveLinStab(Omega, tau, tauf, tauc, v, digital, maxp, expansion):
