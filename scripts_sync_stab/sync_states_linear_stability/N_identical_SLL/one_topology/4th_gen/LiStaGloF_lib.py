...
def solveLinStab(Omega, tau, tauf, Kvco, AkPD,  GkLF,Gvga, tauc, v, digital, maxp, expansion, INV):
	...
	if expansion:
		...
		# print('temp.x[0] =', temp.x[0], 'temp.x[1] =', temp.x[1])
			init = initial_guess();
			# (np.real(lambsolRe[index]), np.real(lambsolIm[index]))
			if lambsolRe[index] >= 0:
				OmegUnst.append(Omega[index]); tauUnst.append(tau[index]);
			else:
				OmegStab.append(Omega[index]); tauStab.append(tau[index]);
...
def solveLinStab_topology_comparison(Omega, tau, tauf, Kvco, AkPD,  GkLF, Gvga, tauc, v, digital, maxp, expansion, zetas, INV):
	...
	if expansion:
		...
		for index1 in range(len(xrange(maxp, tauf))):
			for index2 in range(len(zetas)):
				# print(index, wR[index])
				temp = optimize.root(linStabEq, init, args=(Omega[index1], tau[index1], tauf, Kvco, AkPD,  GkLF,Gvga, tauc, v, zetas[index2], digital, INV), tol=1e-14, method='hybr')
