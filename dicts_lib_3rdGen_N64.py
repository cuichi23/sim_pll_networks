...
def getDicts(Fsim=55):
	...
	dict_net={
		...
		'Ny': 8,																# oscillators in y-direction
		...
		'Tsim': 2.5E6,															# simulation time in multiples of the period
		...
		'phiPerturb': [0 for i in range(64)],									# delta-perturbation on initial state -- PROVIDE EITHER ONE OF THEM! if [] set to zero
		'phiPerturbRot': [],													# delta-perturbation on initial state -- in rotated space
		'phiInitConfig': [0 for i in range(64)],									# phase-configuration of sync state,  []: automatic, else provide list
		'freq_beacons': 0.25,													# frequency of external sender beacons, either a float or a list
		'special_case': 'False', #'timeDepTransmissionDelay',#'False'			# 'False', or 'test_case', 'timeDepInjectLockCoupStr', 'timeDepTransmissionDelay', 'timeDepChangeOfCoupStr', 'distanceDepTransmissionDelay', 'timeDepChangeOfIntrFreq'
	...
	dict_pll={
		'intrF': 1,																# intrinsic frequency in Hz [random.uniform(0.95, 1.05) for i in range(dict_net['Nx']*dict_net['Ny'])]
		'syncF': 1.0443942814145974,#1,#0.980808,								# frequency of synchronized state in Hz -- or the reference frequency of there is a reference
		'coupK': 0.5*0.0500991,													# [random.uniform(0.3, 0.4) for i in range(dict_net['Nx']*dict_net['Ny'])],# coupling strength (like phase model: K = Kvco/2 * G_all, NOTE: the /2 is for coupling functions that have peak2peal amplitude 2) in Hz float or [random.uniform(minK, maxK) for i in range(dict_net['Nx']*dict_net['Ny'])]
		...
		'cutFc': 0.5E-5,#16.5E-6,												# LF cut-off frequency in Hz, here N=9 with mean 0.015: [0.05,0.015,0.00145,0.001,0.0001,0.001,0.00145,0.015,0.05]
		...
		'transmission_delay': 39968,#1.75,#3.05,#1571.805,						# value of transmission delay in seconds, float (single), list (tau_k) or list of lists (tau_kl): np.random.uniform(min,max,size=[dict_net['Nx']*dict_net['Ny'],dict_net['Nx']*dict_net['Ny']]), OR [np.random.uniform(min,max) for i in range(dict_net['Nx']*dict_net['Ny'])]
		...
		'sampleF': Fsim,														# sampling frequency in Hz
	...
	dict_algo={
		'parameter_space_sweeps': 'single',									# pick method for setting realizations 'single', 'classicBruteForceMethodRotatedSpace', 'listOfInitialPhaseConfigurations', 'single', 'statistics', 'two_parameter_sweep', 'one_parameter_sweep'
		...
		'min_max_range_parameter_0': [1-5E-9, 1+5E-9],							# specifies within which min and max value to linspace the, e.g., initial frequency difference (w.r.t. HF Frequency, not divided) in the case of 'listOfInitialPhaseConfigurations', or any other parameter as in, e.g., 'two_parameter_sweep'
