...
c_0		= c;
X= Krange();
Y=globalFreq(w, Kvco, AkPD, Ga1, tauf, v, digital, maxp)['tau'];
Z=np.asarray(w/(2.0*np.pi))*globalFreq(w, Kvco, AkPD, Ga1, tauf, v, digital, maxp)['tau'], np.asarray(2.0*np.pi/w)*solveLinStab(globalFreq(w, Kvco, AkPD, Ga1, tauf, v, digital, maxp)['Omeg'],
	globalFreq(w,Kvco, AkPD, Ga1, tauf, v, digital, maxp)['tau'], tauf_0, Kvco, AkPD, Ga1_0, tauc_0, v_0, digital, maxp, expansion)['ReKraMax'];
ax.contourf(X, Y, Z, 10, cmap=plt.cm.bone, origin=origin)
ax.colorbar();
