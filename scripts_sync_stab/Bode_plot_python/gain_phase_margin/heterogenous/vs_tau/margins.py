...
from bode_lib import solveLinStabbetaanti, solveLinStabbetain, globalFreq, linStabEq, K, coupfunction, equationGamma, equationSigma
...
tausigmaW=[];
OmegsigmaW=[]
for j in range(len(syncStateInphase['Omegabetain']) ):
	gamma=np.max(equationGamma(syncStateInphase['taubetain'][j],
    LoopGainSteadyState1(syncStateInphase['Omegabetain'][j], syncStateInphase['taubetain'][j], syncStateInphase['betain'][j], tauf, v, Kvco1, AkPD, GkLF, Gvga, digital, 'Nonlinear'),
    	1.0/tauchomogen, zeta ) )
	if np.isnan( gamma )== False:
		sigma= np.max(equationSigma(syncStateInphase['taubetain'][j], LoopGainSteadyState1(syncStateInphase['Omegabetain'][j], syncStateInphase['taubetain'][j], syncStateInphase['betain'][j], tauf, v, Kvco1, AkPD, GkLF, Gvga, digital, 'Nonlinear'),
				1.0/tauchomogen, zeta, gamma ))

		if np.isnan( sigma )== False:
			sigmaLambertW.append(sigma)
			tausigmaW.append( syncStateInphase['taubetain'][j])
			OmegsigmaW.append( syncStateInphase['Omegabetain'][j])
...
[lineSigmaWin] = ax7.plot(np.asarray(1.0/(2.0*np.pi))*np.asarray(tausigmaW)*np.asarray(OmegsigmaW), np.asarray(2.0*np.pi/w1)*np.asarray(sigmaLambertW),'d' , ms=6, color='orange', label=r'Lambert-W')
...
[lineSigmaIn] = ax7.plot(np.asarray(syncStateInphase['Omegabetain'])*np.asarray(syncStateInphase['taubetain'])/(2.0*np.pi), np.asarray(2.0*np.pi/wmean)*solveLinStabbetain(syncStateInphase['Omegabetain'],
					...
					'-' , linewidth=4, label=r'in-phase $\Delta \omega_c=0$', color='blue');
...
[lineSigmaInHetero] = ax7.plot(np.asarray(syncStateInphase['Omegabetain'])*np.asarray(syncStateInphase['taubetain'])/(2.0*np.pi), np.asarray(2.0*np.pi/wmean)*solveLinStabbetain(syncStateInphase['Omegabetain'],
					 ...
					 '--' , linewidth=4, label=r'in-phase $\Delta \omega_c=%0.2f\cdot2\pi \textrm{Hz}$' %(Dwc/(2.0*np.pi)), color='green');
...
plt.grid();
plt.ylabel(r' $\frac{2\pi\sigma}{\omega}$', rotation=90, fontsize=46, labelpad=20)
ax7.legend(bbox_to_anchor=(0.7415,0.75), prop=labelfont)
ax7.tick_params(axis='both', which='major', labelsize=35, pad=1)

ax8 = plt.subplot(212)
ax8.set_xlim(OmegTau[0],1.5)
ax8.tick_params(axis='both', which='major', labelsize=35, pad=1)
plt.ylabel(r'gain margin', rotation=90, fontsize=36, labelpad=20)
plt.xlabel(r'$\Omega\tau/2 \pi$', fontsize=60, labelpad=5)
[lineoNetHomog] = ax8.plot(np.array(tauNetHomog)*np.array(OmegaNetHomog)/(2.0*np.pi), np.array(GM_NetHomog), '-', linewidth=4, label=r'in-phase $\Delta \omega_c=0$', color='blue');
[lineoNetHetero] = ax8.plot(np.array(tauNetHetero)*np.array(OmegaNetHetero)/(2.0*np.pi), np.array(GM_NetHetero), '--' , linewidth=4, label=r'in-phase $\Delta \omega_c=%0.2f\cdot2\pi \textrm{Hz}Hz$' %(Dwc/(2.0*np.pi)), color='green');
# [lineoNetHomog_AP] = ax8.plot(np.array(tauNetHomog_AP)*np.array(OmegaNetHomog_AP)/(2.0*np.pi), np.array(GM_NetHomog_AP), 'o' ,mfc='none', ms=4, label=r'anti-phase $\Delta \omega_c=0 $', color='green');
