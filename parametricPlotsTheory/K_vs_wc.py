...
for i in range(len(K)):
	...
	for j in range(len(wc)):
		...
		if len(para_mat[:,4]) > 1:
			...
			#print( 'alpha1: ', alpha[i,j], '\talpha2: ', K[i]/dict_pll['div']*dict_pll['derivative_coup_fct']( (-2.0*np.pi*para_mat[index,4]*tau+beta)/dict_pll['div'] ) )
			...
			#print( 'alpha1: ', alpha[i,j], '\talpha2: ', K[i]/dict_pll['div']*dict_pll['derivative_coup_fct']( (-2.0*np.pi*para_mat[:,4]*tau+beta)/dict_pll['div'] ) )
...
dict_pll.update({'coupK': K/(2*np.pi)})											# set coupling strength key in dict_pll back to the array
