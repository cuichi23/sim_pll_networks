...
if len(para_mat[:,4]) > 1:
	...
	alpha    = ((2.0*np.pi*para_mat[index,1]/para_mat[index,12])*dict_pll['derivative_coup_fct']( (-2.0*np.pi*para_mat[index,4]*para_mat[index,3]+beta)/para_mat[index,12] ))
	...
	alpha    = ((2.0*np.pi*para_mat[:,1]/para_mat[:,12])*dict_pll['derivative_coup_fct']( (-2.0*np.pi*para_mat[:,4]*para_mat[:,3]+beta)/para_mat[:,12] ))[0]
